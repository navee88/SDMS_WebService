...
//   // Add this new navigation function in SchedulerNavigationProvider
//   const navigateFromDataSchedulerToEditTask = useCallback((data) => {
//     console.log('=== navigateFromDataSchedulerToEditTask CALLED ===');
//     console.log('Data from DataScheduler:', data);

//     const navigationData = {
//       innerTab: 'Edit Task',
//       fromDataScheduler: true,
//       scheduleUpdated: true,
//       scheduleId: data?.scheduleId,
//       showSuccess: data?.showSuccess || true,
//       message: data?.message || 'Schedule updated successfully',
//       navigationTimestamp: Date.now()
//     };

//     console.log('Navigating to Edit Task from DataScheduler:', navigationData);
//     navigateToTab('Scheduler', 'View Edit Scheduler', navigationData);
//   }, [navigateToTab]);
...
const navigateFromDeactivatedToActivatedTask = useCallback((data) => {
  ...
  // Force innerTab to Activated Task
  const navigationData = {
    innerTab: 'Activated Task', // Force this
    targetTab: 'Activated Task',
    fromDeactivatedTask: true,
    scheduleActivated: true,
    scheduleId: data?.scheduleId || data?.data?.scheduleId,
    highlightScheduleId: data?.highlightScheduleId || data?.scheduleId,
    shouldScrollToSchedule: data?.shouldScrollToSchedule || true,
    message: data?.message || 'Schedule activated successfully',
    fromInstrumentLock: data?.fromInstrumentLock || false,
    lockCompleted: data?.lockCompleted || false,
    ...data,
    navigationTimestamp: Date.now()
  };
  ...
  // Force the navigation with explicit parameters
  setNavigationState({
    targetTab: 'View Edit Scheduler', // Parent tab
    targetComponent: 'View Edit Scheduler',
    parentTabKey: 'Scheduler',
    childTabKey: 'View Edit Scheduler',
    data: navigationData,
    timestamp: Date.now(),
    forceInnerTab: 'Activated Task' // Force parameter
  });
  
  setActiveParentTab('Scheduler');
  setActiveChildTab('View Edit Scheduler');
  ...
  window.dispatchEvent(new CustomEvent('tab-navigation', {
    detail: { 
      parentTabKey: 'Scheduler', 
      childTabKey: 'View Edit Scheduler',
      innerTab: 'Activated Task', // Specify inner tab
      data: navigationData,
      forceShowActivated: true
    }
  }));
  ...
  // In SchedulerNavigationContext.jsx, add this useEffect:

useEffect(() => {
  const handleInstrumentLockCompleted = (event) => {
    console.log('📢 Received instrument-lock-completed event:', event.detail);
    
    const { scheduleId, activateData, fromInstrumentLock, shouldNavigateToActivatedTask } = event.detail || {};
    
    if (fromInstrumentLock && shouldNavigateToActivatedTask && scheduleId) {
      console.log('🚀 Instrument lock completed, navigating to Activated Task for schedule:', scheduleId);
