...
// Hide Empty Folder is REMOVED from this list (controlled by API only)
const CUSTOM_FILTERS = ["Instrument", "Workflow Status", "Task Status"]; 
const CUSTOM_COLUMNS = ["Parser Status"];
...
export default function ConfigModal({ onClose, currentVisibility, onSave, showDialog }) {
  ...
  const handleSubmit = () => {
    onSave(tempVisibility);
    // Modal closing is handled by parent after API success
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
        ...
        >
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t bg-slate-50/50 rounded-b-md">
            <button onClick={handleSubmit} className="px-3 py-2 bg-blue-600 text-white font-medium text-sm rounded hover:bg-blue-700 flex items-center gap-2 shadow-sm">
              Submit
            </button>
            <button onClick={onClose} className="px-3.5 py-2 bg-white border text-sm border-slate-300 text-slate-600 font-medium rounded hover:bg-slate-50 shadow-sm">
              Close
            </button>
          </div>
        </div>
