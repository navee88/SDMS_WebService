...
import {
  ...
  getCurrentDate,
} from "./Templatesample/Constantdata";
...
export default function TemplateView() {
  ...
  useEffect(() => {
    // Force a calculation after a short delay to ensure DOM is ready
    const timer = setTimeout(() => {
      calculateVisibleActions();
    }, 100);

    // Recalculate whenever window width changes
    calculateVisibleActions();
  ...
  }, [calculateVisibleActions,width]); // Added 'width' dependency
  ...
  const getIsActionDisabled = useCallback(
    (actionName) => {
      if (selectedRow)
        return !["Open", "Restore", "Folder Download", "File Upload", "Folder Upload","Tag"].includes(
          actionName
        );
      if (hasFiltered) return !["Open", "Restore"].includes(actionName);
      return ["Version History", "Work Complete"].includes(actionName);
    },
  ...
  return (
    <div className="flex flex-col w-full font-sans rounded-md relative h-full">
      ...
      {/* FILTER BAR */}
      <div className="bg-[#f0f4f8] px-4 pt-4 pb-2 relative rounded-t-md z-10">
        {isFilterOpen ? (
