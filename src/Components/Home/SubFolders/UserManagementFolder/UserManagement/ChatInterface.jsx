...
useEffect(() => {
    if (messagesEndRef.current && initialLoadRef.current && messages.length > 0) {
        ...
        initialLoadRef.current = false; // Set to false after first scroll
    }
}, [messages,loading]); // This will run when messages change
    ...
    if (!newMessage.trim()) {
        setMessageError(true); // Show error for empty message
        return;
    }
    
    if (!selectedUser) return;
    
    // Reset error state if message is valid
...
}, [newMessage, selectedUser, postData, getApiRequestData, fetchChatHistory, showInfoDialogLocal, t, messageError]); // Add messageError here // Add messageError to dependencies
    ...
    }, [messages, postData, getApiRequestData, showInfoDialogLocal, t]);
...
<div className="flex-1 flex p-4 gap-2 overflow-hidden">
    {/* Messages Inbox - 65% width (exact width from image) */}
   ...
   <div className="space-y-3">
    {messages.map((message) => (
        ...
        >
            ...
            >
                ...
                {/* Message text - Wrapped and no overflow */}
                <div 
    ...
    {/* Send Message Panel - 35% width (exact width from image) */}
