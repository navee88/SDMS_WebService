...
//           console.log(`Auto-selecting first client: ${firstClient.label} (${firstClient.value})`);
...
//   }, [formData, clientOptions, instrumentOptions, pathOptions, tags]);
...
const performLockAction = useCallback(async (auditData) => {
    try {
      ...
      if (response && response.Rtn && response.Rtn.toLowerCase() === "success") {
        ...
        // 4. Use the prop callback instead of navigate
        setTimeout(() => {
          if (onNavigateToMyInstruments) {
            onNavigateToMyInstruments(); // This will trigger navigation to MyInstruments
          }
        }, 500);
        
      } else {
        throw new Error(response?.Message || 'Failed to lock instrument');
      }
