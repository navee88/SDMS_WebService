    ...
    const handleViewSchedule = useCallback(async () => {
        ...
        try {
            ...
            if (response && (response.ViewDatas || response.ViewLoad)) {
                ...
                // Use the new navigation function
                if (navigateFromDeactivatedToDataScheduler) {
                    navigateFromDeactivatedToDataScheduler(navigationPayload);
                } else if (navigateToTab) {
                    navigateToTab('Scheduler', 'Data Scheduler', navigationPayload);
                }
        ...
        navigateFromDeactivatedToDataScheduler, navigateToTab, showInfoDialog]);
...
const fetchLIMSOrders = async (instrumentMappingId) => {
  try {
    // If instrument is interface, fetch LIMS orders
    const response = await makeApiCall(
      endpoints.lockLimsordercombo, // Use appropriate endpoint
      {
        nInterfaceInstID: instrumentMappingId
      },
      "FetchLIMSOrders"
    );
    
    if (Array.isArray(response) && response.length > 0) {
      // Return the first LIMS order
      return response[0];
    }
    return null;
  } catch (error) {
    console.error('Error fetching LIMS orders:', error);
    return null;
  }
};

const navigateToInstrumentLock = useCallback((data) => {
  console.log("🔵 === NAVIGATE TO INSTRUMENT LOCK ===");
  console.log("🔵 Full navigation data:", JSON.stringify(data, null, 2));
  
  // Extract LIMS order data from checkResponse
  const limsOrderData = data.checkResponse?.LIMSobj;
  
  console.log('📊 LIMS order data from checkResponse:', limsOrderData);
  console.log('📊 Has LIMS data:', !!limsOrderData);
  console.log('📊 checkResponse keys:', data.checkResponse ? Object.keys(data.checkResponse) : 'No checkResponse');
  
  // Prepare navigation data with LIMS order
  const navigationData = {
    scheduleData: data.scheduleData,
    checkResponse: data.checkResponse,
    actionType: data.actionType || 'lockActivate',
    ...
    activateData: data.activateData,
    LIMSobj: limsOrderData,
    navigationTimestamp: Date.now()
  };
  
  console.log('📤 Final navigation payload:', JSON.stringify(navigationData, null, 2));
  
  // Store in session storage
  console.log('💾 Storing in sessionStorage...');
  sessionStorage.setItem('deactivatedTaskNavigation', JSON.stringify(navigationData));
  ...
  // Use navigation
  if (navigateFromDeactivatedToInstrumentLock) {
    console.log('📍 Using navigateFromDeactivatedToInstrumentLock');
    navigateFromDeactivatedToInstrumentLock(navigationData);
  } else if (navigateToTab) {
    console.log('📍 Using navigateToTab fallback');
    navigateToTab('Lock Settings', 'Instrument Lock Settings', navigationData);
  }
  
  console.log("✅ === NAVIGATION INITIATED ===");
}, [navigateFromDeactivatedToInstrumentLock, navigateToTab]);    // Add event listeners for lock completion
    useEffect(() => {
        // Listen for when we need to navigate back from Instrument Lock
        const handleNavigateBackForActivation = (event) => {
            console.log('🔄 Received navigateToDeactivatedForActivation event:', event.detail);
    ...
    const testNavigation = useCallback(() => {
        ...
        // Create lock data inside the function where selectedScheduler exists
        const lockSchedulerData = {
    // Client Information
    clientId: selectedScheduler.L06ClientID || '',
    L06ClientID: selectedScheduler.L06ClientID || '',
    L06ClientName: selectedScheduler.L06ClientName || '',
    ...
    instrumentName: selectedScheduler.L11InstrumentAliasName || '',
    dropdownInstrumentId: selectedScheduler.L13InstrumentMappingID || selectedScheduler.L12InstrumentMappingID || '',
    L13InstrumentMappingID: selectedScheduler.L13InstrumentMappingID || '',
    L12InstrumentMappingID: selectedScheduler.L12InstrumentMappingID || '',
    L11InstrumentAliasName: selectedScheduler.L11InstrumentAliasName || '',
    ...
    L52TaskID: selectedScheduler.L52TaskID || '',
    sTaskID: selectedScheduler.L52TaskID || '',
    L13TaskID: selectedScheduler.L13ScheduleID || '',  // ← IMPORTANT: For taskType detection
    ...
    // Status Information
    TaskType: 'DeActive',  // ← This tells InstrumentLockTag it's a deactivated task
    TaskStatus: selectedScheduler.TaskStatus || 'D',
    
    // Additional context (helps InstrumentLockTag understand the flow)
    actionType: 'lockActivate',
    forActivation: true,
    bypassRightsCheck: true
};
...
const handleActivateConfirm = useCallback(async () => {
  console.log("🔵 === ACTIVATE CLICK STARTED ===");
  console.log("🔵 Selected Scheduler:", selectedScheduler);
  
  if (!selectedScheduler) {
    console.log("🔴 No scheduler selected, returning");
    return;
  }
  
  try {
    // STEP 1: Prepare checkManualTask API request
    const checkRequestData = prepareRequestBody({
      sTaskID: selectedScheduler.L13ScheduleID,
      sTaskStatus: selectedScheduler.TaskStatus,
      sPathTaskID: selectedScheduler.L52TaskID,
    ...
    console.log("📤 === STEP 1: Sending checkManualTask API Request ===");
    console.log("📤 Request URL: checkManualTask endpoint");
    console.log("📤 Request Data:", JSON.stringify(checkRequestData, null, 2));
    console.log("📤 Selected Schedule ID:", selectedScheduler.L13ScheduleID);
    console.log("📤 Task Status:", selectedScheduler.TaskStatus);
    console.log("📤 Path Task ID:", selectedScheduler.L52TaskID);
    console.log("📤 Instrument Mapping ID:", selectedScheduler.L13InstrumentMappingID || selectedScheduler.L12InstrumentMappingID);
    
    // STEP 2: Make API call to checkManualTask
    console.log("⏳ Calling makeApiCall for checkManualTask...");
    const checkResponse = await makeApiCall(
      endpoints.checkManualTask,
      checkRequestData,
      "CheckManualTask"
    ...
    console.log("📥 === STEP 2: Received checkManualTask API Response ===");
    console.log("📥 Full Response:", JSON.stringify(checkResponse, null, 2));
    console.log("📥 Response Type:", typeof checkResponse);
    console.log("📥 Response Keys:", checkResponse ? Object.keys(checkResponse) : 'No response');
    console.log("📥 Critical Values:");
    console.log("   - nTaskCount:", checkResponse?.nTaskCount);
    console.log("   - nParsingCount:", checkResponse?.nParsingCount);
    console.log("   - nParsingInstrOrderCount:", checkResponse?.nParsingInstrOrderCount);
    console.log("   - Has LIMSobj:", !!checkResponse?.LIMSobj);
    console.log("   - LIMSobj data:", checkResponse?.LIMSobj);
    
    if (checkResponse && checkResponse.nTaskCount !== undefined) {
      // STEP 3: Prepare activation data
      const activateData = {
        sClientName: selectedScheduler.L06ClientName,
        sTaskID: selectedScheduler.L13ScheduleID,
        sTaskStatus: selectedScheduler.TaskStatus,
      ...
      console.log("📝 === STEP 3: Prepared Activation Data ===");
      console.log("📝 Activate Data:", JSON.stringify(activateData, null, 2));
      
      // STEP 4: Prepare lock scheduler data
      const lockSchedulerData = {
        clientId: selectedScheduler.L06ClientID || selectedScheduler.L06ClientName,
        L06ClientID: selectedScheduler.L06ClientID || selectedScheduler.L06ClientName,
        instrumentId: selectedScheduler.L13InstrumentMappingID || selectedScheduler.L12InstrumentMappingID,
        L11InstrumentID: selectedScheduler.L13InstrumentMappingID || selectedScheduler.L12InstrumentMappingID,
        instrumentName: selectedScheduler.L11InstrumentAliasName,
        dropdownInstrumentId: selectedScheduler.L13InstrumentMappingID || selectedScheduler.L12InstrumentMappingID,
        ...
        fromLockActivate: true,
        fromDeactivatedTask: true,
        TaskType: selectedScheduler.TaskType || 'DeActive',
        TaskStatus: selectedScheduler.TaskStatus || 'D'
      };
      
      console.log("🔒 === STEP 4: Prepared Lock Scheduler Data ===");
      console.log("🔒 Lock Data:", JSON.stringify(lockSchedulerData, null, 2));
      
      // Check for LIMS data
      let limsOrderData = null;
      const isInterface = checkResponse.nParsingInstrOrderCount > 0;
      
      if (isInterface) {
        console.log("🔍 Interface instrument detected!");
        console.log("🔍 LIMSobj in checkResponse:", checkResponse?.LIMSobj);
        
        if (checkResponse.LIMSobj) {
          limsOrderData = checkResponse.LIMSobj;
          console.log("✅ Found LIMS order data in checkResponse");
        } else {
          console.log("⚠️ No LIMS order data in checkResponse");
        }
      }
      
      // STEP 5: Build lock activate payload
      const lockActivatePayload = {
        scheduleData: lockSchedulerData,
        checkResponse: {
          ...checkResponse,
      ...
      console.log("📦 === STEP 5: Built Lock Activate Payload ===");
      console.log("📦 Full Payload:", JSON.stringify(lockActivatePayload, null, 2));
      
      // Store in sessionStorage
      console.log("💾 Storing in sessionStorage...");
      sessionStorage.setItem('lockAndActivateFlow', JSON.stringify({
        scheduleData: lockSchedulerData,
        activateData: activateData,
        fromLockActivate: true,
        fromDeactivatedTask: true
      ...
      // STEP 6: Handle different scenarios based on API response
      console.log("🎯 === STEP 6: Evaluating Scenario ===");
      console.log("🎯 nTaskCount:", checkResponse.nTaskCount);
      console.log("🎯 nParsingCount:", checkResponse.nParsingCount);
      console.log("🎯 nParsingInstrOrderCount:", checkResponse.nParsingInstrOrderCount);
      ...
      // SCENARIO: nTaskCount === 1
      if (checkResponse.nTaskCount === 1) {
        console.log("🎯 SCENARIO: nTaskCount === 1 (Single task)");
        
        // Sub-scenario 1: Instrument already locked
        if (checkResponse.nParsingInstrOrderCount >= 1) {
          console.log("🎯 SUB-SCENARIO 1: Instrument already locked (nParsingInstrOrderCount >= 1)");
          console.log("🎯 Showing confirmation for activation only...");
          
          showConfirmation(
            t('scheduler.confirmation'),
            t('scheduler.confirmActivateWithLockInfo'),
            () => {
              console.log("✅ User clicked confirm for activation (instrument already locked)");
              ...
              if (auditTrailRights.activate === 1) {
                ...
                console.log("⚡ No audit trail required, executing activate...");
                executeActivate(activateData);
              }
            },
            "activate"
        ...
        // Sub-scenario 2: Manual parsing instrument - needs lock
        else if (checkResponse.nParsingCount === 1) {
          console.log("🎯 SUB-SCENARIO 2: Manual parsing instrument (nParsingCount === 1)");
          console.log("🎯 Showing lock & activate dialog...");
          ...
          setConfirmDialogData({
            ...
            onConfirm: (actionType) => {
              ...
              if (actionType === 'lockActivate') {
                console.log("🚀 === LOCK & ACTIVATE FLOW STARTED ===");
                
                // Create fresh lock data for navigation
                const freshLockSchedulerData = {
                  ...lockSchedulerData,
                  navigationTimestamp: Date.now()
                };
                
                const lockActivatePayload = {
                  scheduleData: freshLockSchedulerData,
                  checkResponse: {
                    ...checkResponse,
                    LIMSobj: limsOrderData
                  },
                ...
                console.log("📤 Navigation Payload:", JSON.stringify(lockActivatePayload, null, 2));
                ...
                console.log('   - lockAndActivateFlow:', JSON.parse(sessionStorage.getItem('lockAndActivateFlow')));
                console.log('   - originalScheduleId:', sessionStorage.getItem('originalScheduleId'));
                
                console.log("📍 Calling navigateToInstrumentLock...");
        ...
        // Sub-scenario 3: Simple activation (no parsing, no lock)
          ...
          console.log("🎯 SUB-SCENARIO 3: Simple activation (no parsing needed)");
          console.log("🎯 Showing confirmation for simple activation...");
          
          showConfirmation(
            t('scheduler.confirmation'),
            t('scheduler.confirmActivate'),
            () => {
              console.log("✅ User clicked confirm for simple activation");
      ...
      // SCENARIO: nTaskCount === 0 (pending manual upload)
        ...
        console.log("🎯 SCENARIO: nTaskCount === 0 (Pending manual upload)");
        console.log("🎯 Showing confirmation for activation with manual upload...");
        
        showConfirmation(
          t('scheduler.confirmation'),
          t('scheduler.confirmActivateWithManual'),
          () => {
            console.log("✅ User clicked confirm for activation with manual upload");
        ...
        console.log("❓ Unknown scenario - nTaskCount:", checkResponse.nTaskCount);
      }
      
      console.log("✅ === ACTIVATE CLICK PROCESS COMPLETED ===");
    } else {
      console.log("🔴 Invalid API response or missing nTaskCount");
    }
  } catch (error) {
